#!/bin/bash
# Deploy NachoMarket al VPS Oracle Cloud (o cualquier servidor Linux)
#
# Uso:
...
if [ -z "$VPS" ]; then
    ...
    echo ""
    echo "Ejemplos:"
    echo "  $0 ubuntu@129.146.45.123           # Solo sync de archivos"
    echo "  $0 ubuntu@129.146.45.123 --setup   # Sync + setup completo + arrancar bot"
    exit 1
fi
...
if [ "$SETUP_FLAG" = "--setup" ]; then
    ...
    ssh "$VPS" "cd $REMOTE_DIR && bash scripts/setup_oracle_vm.sh"
    echo ""
    echo "✅ Setup completo."
fi
...
if [ "$SETUP_FLAG" != "--setup" ]; then
    echo "  3. bash scripts/setup_oracle_vm.sh    # Solo la primera vez"
fi

echo "  4. nano .env                           # Pegar API keys"
echo "  5. python scripts/check_geo.py         # Verificar acceso geo"
echo "  6. sudo systemctl start polymarket-bot # Arrancar bot"
echo "  7. sudo journalctl -u polymarket-bot -f # Ver logs"
echo ""
